...
if ( ! function_exists( 'kastell_mkdf_get_hide_dep_for_header_logo_area_meta_boxes' ) ) {
	function kastell_mkdf_get_hide_dep_for_header_logo_area_meta_boxes() {
		$hide_dep_options = apply_filters( 'kastell_mkdf_header_logo_area_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'kastell_mkdf_header_logo_area_meta_options_map' ) ) {
	function kastell_mkdf_header_logo_area_meta_options_map( $header_meta_box ) {
		$hide_dep_options = kastell_mkdf_get_hide_dep_for_header_logo_area_meta_boxes();
		...
		kastell_mkdf_add_admin_section_title(
			array(
				...
				'title'  => esc_html__( 'Logo Area Style', 'kastell' )
			)
		);
		...
		kastell_mkdf_create_meta_box_field(
			array(
				...
				'type'          => 'yesno',
				'default_value' => 'no',
				'label'         => esc_html__( 'Disable Header Logo Area Widget', 'kastell' ),
				'description'   => esc_html__( 'Enabling this option will hide widget area from the logo area', 'kastell' ),
				'parent'        => $logo_area_container
			)
		);
		...
		if ( count( $kastell_custom_sidebars ) > 0 ) {
			kastell_mkdf_create_meta_box_field(
				array(
					...
					'type'        => 'selectblank',
					'label'       => esc_html__( 'Choose Custom Widget Area for Logo Area', 'kastell' ),
					'description' => esc_html__( 'Choose custom widget area to display in header logo area"', 'kastell' ),
					'parent'      => $logo_area_container,
		...
		kastell_mkdf_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Set grid background transparency for logo area (0 = fully transparent, 1 = opaque)', 'kastell' ),
				'parent'      => $logo_area_in_grid_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		kastell_mkdf_create_meta_box_field(
			array(
				...
				'description'   => esc_html__( 'Set border on grid logo area', 'kastell' ),
				'parent'        => $logo_area_in_grid_container,
				'default_value' => '',
				'options'       => kastell_mkdf_get_yes_no_select_array(),
				'args'          => array(
		...
		kastell_mkdf_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a background color for logo area', 'kastell' ),
				'parent'      => $logo_area_container
			)
		);
		...
		kastell_mkdf_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a transparency for the logo area background color (0 = fully transparent, 1 = opaque)', 'kastell' ),
				'parent'      => $logo_area_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		$logo_area_border_bottom_color_container = kastell_mkdf_add_admin_container(
			array(
				'type'            => 'container',
		...
		do_action( 'kastell_mkdf_header_logo_area_additional_meta_boxes_map', $logo_area_container );
	}
	
	add_action( 'kastell_mkdf_header_logo_area_meta_boxes_map', 'kastell_mkdf_header_logo_area_meta_options_map', 10, 1 );
}