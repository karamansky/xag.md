...
namespace KastellMkdf\Modules\Header\Lib;
/**
 * Class KastellMkdfHeaderConnector
 *
...
class KastellMkdfHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'kastell_mkdf_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'kastell_mkdf_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'kastell_mkdf_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'kastell_mkdf_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to kastell_mkdf_title_content_padding filter
	 *
	 * @return int
	 */
