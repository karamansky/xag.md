...
class KastellMkdfLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_kastell_mkdf_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_kastell_mkdf_like', array( $this, 'ajax' ) );
	}
		...
		switch ( $action ) {
			...
			case 'get':
				...
				if ( isset( $_COOKIE[ 'mkdf-like_' . $post_id ] ) ) {
					$icon = '<i class="dripicon dripicons-heart" aria-hidden="true"></i>';
				} else {
					$icon = '<i class="dripicon dripicons-heart" aria-hidden="true"></i>';
				}
			...
			case 'update':
				...
				$return_value = "<i class='dripicon dripicons-heart' aria-hidden='true'></i><span>" . esc_attr( $like_count ) . "</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'kastell' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'kastell' );
		}
...
if ( ! function_exists( 'kastell_mkdf_create_like' ) ) {
	function kastell_mkdf_create_like() {
		KastellMkdfLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'kastell_mkdf_create_like' );
}