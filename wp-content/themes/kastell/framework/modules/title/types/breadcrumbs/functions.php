...
if ( ! function_exists( 'kastell_mkdf_set_title_breadcrumbs_type_for_options' ) ) {
	/**
	 * This function set breadcrumbs title type value for title options map and meta boxes
	 */
	function kastell_mkdf_set_title_breadcrumbs_type_for_options( $type ) {
		$type['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'kastell' );
		
		return $type;
	}
	
	add_filter( 'kastell_mkdf_title_type_global_option', 'kastell_mkdf_set_title_breadcrumbs_type_for_options' );
	add_filter( 'kastell_mkdf_title_type_meta_boxes', 'kastell_mkdf_set_title_breadcrumbs_type_for_options' );
}

if ( ! function_exists( 'kastell_mkdf_custom_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs
	 */
	function kastell_mkdf_custom_breadcrumbs() {
		global $post;
		
		$pageid    = kastell_mkdf_get_page_id();
		$homeLink  = esc_url( home_url( '/' ) );
		$home      = esc_html__( 'Home', 'kastell' );
		...
		$output = '<div itemprop="breadcrumb" class="mkdf-breadcrumbs ' . esc_attr( $holder_classes ) . '" ' . kastell_mkdf_get_inline_attr( $holder_styles, 'style', ';' ) . '>';
			...
			if ( is_home() && ! is_front_page() ) {
				...
				if ( is_tag() ) {
					$childContent .= $before . esc_html__( 'Posts tagged ', 'kastell' ) . '"' . single_tag_title( '', false ) . '"' . $after;
					...
					$childContent    .= $before . esc_html__( 'Articles posted by ', 'kastell' ) . get_the_author_meta( 'display_name', $author_id ) . $after;
					...
					$childContent .= $before . esc_html__( 'Search results for ', 'kastell' ) . '"' . get_search_query() . '"' . $after;
					
				} elseif ( is_404() ) {
					$childContent .= $before . esc_html__( 'Error 404', 'kastell' ) . $after;
				...
				if ( get_query_var( 'paged' ) ) {
					$childContent .= $before . " (" . esc_html__( 'Page', 'kastell' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
					
				}
				
				$output .= apply_filters( 'kastell_mkdf_breadcrumbs_title_child_output', $childContent, $delimiter, $before, $after );
			}
		
		$output .= '</div>';
		
		echo wp_kses( apply_filters( 'kastell_mkdf_breadcrumbs_title_output', $output ), array(
			'div'  => array(
				'itemprop' => true,
				'id'       => true,
				'class'    => true,
