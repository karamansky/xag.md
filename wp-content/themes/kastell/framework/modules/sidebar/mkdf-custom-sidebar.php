 ...
 */
if ( ! class_exists( 'KastellMkdfSidebar' ) ) {
	class KastellMkdfSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'kastell' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'kastell-mkdf-sidebar', MIKADO_ROOT . '/framework/admin/assets/js/mkdf-sidebar.js' );
			wp_enqueue_style( 'kastell-mkdf-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		
		//widget form template
		function template_add_widget_field() {
			$nonce = wp_create_nonce('mkdf-delete-sidebar');
			$nonce = '<input type="hidden" name="mkdf-delete-sidebar" value="' . $nonce . '" />';
			
			echo "\n<script type='text/html' id='mkdf-add-widget'>";
			echo "\n  <form class='mkdf-add-widget' method='POST'>";
			echo "\n  <h3>" . esc_html($this->title) . "</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'kastell') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'kastell') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'kastell_mkdf_custom_widget_args', $args );
