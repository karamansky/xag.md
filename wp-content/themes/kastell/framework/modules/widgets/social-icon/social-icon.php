...
class KastellMkdfSocialIconWidget extends KastellMkdfWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_social_icon_widget',
			esc_html__( 'Mikado Social Icon Widget', 'kastell' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'kastell' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			kastell_mkdf_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . kastell_mkdf_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo kastell_mkdf_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php kastell_mkdf_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
