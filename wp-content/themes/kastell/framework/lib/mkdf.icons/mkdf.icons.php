...
/*
  Class: KastellMkdfIconCollections
...
class KastellMkdfIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new KastellMkdfIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new KastellMkdfIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new KastellMkdfIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new KastellMkdfLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new KastellMkdfLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new KastellMkdfSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new KastellMkdfDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'kastell'),
            'font_elegant'      => esc_html__('Font Elegant', 'kastell'),
            'ion_icons'         => esc_html__('Ion Icons', 'kastell'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'kastell')
        );
        if ($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'kastell'),
                'font_elegant'      => esc_html__('Font Elegant', 'kastell'),
                'ion_icons'         => esc_html__('Ion Icons', 'kastell'),
                'linea_icons'       => esc_html__('Linea Icons', 'kastell'),
                'linear_icons'      => esc_html__('Linear Icons', 'kastell'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'kastell'),
                'dripicons'         => esc_html__('Dripicons', 'kastell')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $mkd_icon_pack_container = kastell_mkdf_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'kastell'),
        ...
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'kastell'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'kastell'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'kastell'),
                'font_elegant'      => esc_html__('Font Elegant', 'kastell'),
    ...
    public function getIconCollectionIcons(iKastellMkdfIconCollection $collection) {
    ...
    public function getIconCollectionsEmpty($no_empty_key = "") {
        $array = array();
        $array[$no_empty_key] = esc_html__( 'No Icon', 'kastell');
        foreach ($this->iconCollections as $key => $collection) {
            $array[$key] = $collection->title;
        }
    ...
    public function getIconCollectionsVCEmpty() {
        $vc_array = array();
	    $vc_array[ esc_html__( 'No Icon', 'kastell' ) ] = '';
        foreach ($this->iconCollections as $key => $collection) {
            $vc_array[$collection->title] = $key;
        }
...
if (!function_exists('kastell_mkdf_activate_theme_icons')) {
    function kastell_mkdf_activate_theme_icons() {
        global $kastell_mkdf_IconCollections;
        $kastell_mkdf_IconCollections = KastellMkdfIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'kastell_mkdf_activate_theme_icons');
}