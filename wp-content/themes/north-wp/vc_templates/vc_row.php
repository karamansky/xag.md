...
$disable_element = $bg_video_overlay = '';
$output = $after_output = '';
...
$onepage = is_page_template('template-snap.php');
...
/* Midnight */
if ( $thb_color ) {
	$wrapper_attributes[] = 'data-midnight="' . esc_attr( $thb_color ) . '"';
}
/* Video BG */
if ( $thb_video_bg !== '' ) {
	$bg_type = '';
...
$output .= $onepage ? '<div class="inner-wrap row max_width">'.wpb_js_remove_wpautop( $content ).'</div>' : wpb_js_remove_wpautop( $content );
