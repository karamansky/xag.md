...
function thb_TheShortcodesForVC() {
	if ( function_exists( 'visual_composer' ) ) {
		...
		function thb_column_offset_class_merge( $class_string, $tag ) {
			...
			} elseif ( 'vc_section' === $tag ) {
				$class_string = preg_replace( '/vc_section/', 'vc_section thb_section', $class_string );
			}
			return $class_string;
		}
		...
		class ThbLoopQueryBuilder extends VcLoopQueryBuilder {
			...
			function parse_post_format( $value ) {
				if ( $value ) {
		...
		function thb_hotspot_image( $param, $value ) {
			...
			$output    .= '<div class="thb-hotspot-image-holder no-img" data-popup-title="' . esc_attr__( 'Hotspot Tooltip Content', 'north' ) . '" data-save-text="' . esc_attr__( 'Save changes', 'north' ) . '" data-close-text="' . esc_attr__( 'Close', 'north' ) . '"></div>';
