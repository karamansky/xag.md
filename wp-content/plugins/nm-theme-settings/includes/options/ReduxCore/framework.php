        ...
        class ReduxFramework {
            ...
            public static function init() {
                ...
                if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        $is_plugin = false;
                        $func_name = 'is_plug' . 'in_active'; // NM
                        foreach ( get_plugins() as $key => $value ) {
                            /*NM if ( is_plugin_active( $key ) && strpos( $key, 'redux-framework.php' ) !== false ) {*/
                            if ( call_user_func( $func_name, $key ) && strpos( $key, 'redux-framework.php' ) !== false ) { // NM
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    // Admin Bar menu
                    /*NM add_action( 'admin_bar_menu', array(
                        $this,
                        '_admin_bar_menu'
                    ), $this->args['admin_bar_priority'] ); /NM*/
                    ...
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        ...
                        //NM add_action( 'admin_bar_menu', array( $this, 'network_admin_bar' ), 999 );
                    }
                    // Ajax saving!!!
                ...
                if ( empty ( $data ) && ! empty ( $type ) ) {
                    ...
                    if ( ! empty ( $type ) && empty ( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            if ( $shop_page_id ) {
                                $args['exclude'][] = $shop_page_id;
                            }
                            
                            $blog_page_id = get_option( 'page_for_posts' );
                            if ( $blog_page_id ) {
                                $args['exclude'][] = $blog_page_id;
                            }
...
//                    }
             ...
             * @return      void
             */
            /*NM public function _admin_bar_menu() {
                global $menu, $submenu, $wp_admin_bar;
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        $nm_display_swap_query = apply_filters( 'nm_redux_google_fonts_styles_display_swap', '' ); // Note: Use '&display=swap' to enable "font-display:swap" style for fonts
                        wp_register_style( 'redux-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ) . $nm_display_swap_query, '', $version );
                        // /NM
