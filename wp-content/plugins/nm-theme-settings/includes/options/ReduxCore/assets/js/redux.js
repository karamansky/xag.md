...
(function( $ ) {
    ...
    $.redux.get_container_value = function( id ) {
        ...
        // NM: Image select - Get value from selected radio-input 
        if ( $( '#' + redux.args.opt_name + '-' + id ).hasClass( 'redux-container-image_select' ) ) {
            value = $( '#' + redux.args.opt_name + '-' + id ).find('.redux-image-select-selected input').attr('value');
        }
        // /NM
        return value;
    };
    ...
    $(
        function() {
            if ( redux.rAds ) {
                ...
                $( rAds ).hide();
                rAds.on( "DOMSubtreeModified", function() {
